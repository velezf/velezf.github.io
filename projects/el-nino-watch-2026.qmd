...
# NOAA CRW Bleaching Alert Area (BAA) definitions, per the CRW methodology page:
#   No Stress          HotSpot <= 0
#   Bleaching Watch    0 < HotSpot < 1
#   Bleaching Warning  HotSpot >= 1 and DHW < 4
#   Alert Level 1      HotSpot >= 1 and  4 <= DHW <  8
#   Alert Level 2      HotSpot >= 1 and  8 <= DHW < 12
#   Alert Level 3      HotSpot >= 1 and 12 <= DHW < 16
#   Alert Level 4      HotSpot >= 1 and 16 <= DHW < 20
#   Alert Level 5      HotSpot >= 1 and       DHW >= 20
#
# The Virtual Station files ship NOAA's own classification in the BAA_7day_max
# column. As of 2026 that column still uses the legacy 4-level codes
# (0 No Stress, 1 Watch, 2 Warning, 3 Alert Level 1, 4 Alert Level 2) and
# never reports Levels 3-5, even at DHW > 20. The status table therefore
# takes No Stress through Alert Level 2 from the BAA column, and when the
# column is at its cap (code 4) extends the level to 3-5 from DHW using the
# thresholds above. dhw_to_alert() is also the fallback when BAA is missing.
BAA_LABELS = {
    0: "No Stress",
    1: "Bleaching Watch",
    2: "Bleaching Warning",
...
ALERT_RANK = ["No Stress", "Bleaching Watch", "Bleaching Warning",
...
def dhw_to_alert(dhw, hotspot=None):
    ...
    following the CRW BAA definitions above.

    Watch vs. Warning is decided by HotSpot reaching the 1 deg C bleaching
    threshold, not by DHW. Once HotSpot >= 1, the accumulated DHW sets the
    level in 4 deg C-week steps. If HotSpot is unavailable, DHW alone is used
    and the Watch/Warning split cannot be resolved (returns Watch for DHW = 0,
    Warning for 0 < DHW < 4).
    """
    if pd.isna(dhw):
        return "No Stress"
    if hotspot is not None and not pd.isna(hotspot):
        if hotspot <= 0:
            return "No Stress"
        if hotspot < 1:
...
def baa_to_alert(baa, dhw=None, hotspot=None):
    """
    Return the alert level string for a row, preferring NOAA's own BAA_7day_max
    code. Because the Virtual Station column caps at legacy Alert Level 2
    (code 4), a capped row is extended to Levels 3-5 using the DHW-based
    classifier. Falls back to dhw_to_alert() when the BAA value is missing.
    """
    if baa is None or pd.isna(baa) or int(baa) not in BAA_LABELS:
        return dhw_to_alert(dhw, hotspot)
    label = BAA_LABELS[int(baa)]
    if int(baa) == 4:
        extended = dhw_to_alert(dhw, hotspot)
        if ALERT_RANK.index(extended) > ALERT_RANK.index(label):
            label = extended
    return label
...
baseline. The `alert` column is NOAA's own Bleaching Alert Area classification
(`BAA_7day_max`). That column still uses the legacy four-level codes (0 through 4,
No Stress through Alert Level 2) and never reports Levels 3 to 5, so
`baa_to_alert()` takes the lower levels straight from NOAA and, when the column is
at its Level 2 cap, extends to Levels 3 to 5 from DHW using the same NOAA thresholds
implemented in `dhw_to_alert()`.
...
for name, df in site_data.items():
    last = df.iloc[-1]
    alert = baa_to_alert(last.get("alert"), last["dhw"], last.get("hotspot"))
    rows.append({
        "Site":              name,
...
rolling 84-day window. The alert level is NOAA CRW's own Bleaching Alert Area
classification (`BAA_7day_max`, the highest level reached in the past 7 days),
extended from its legacy Alert Level 2 cap to Levels 3 to 5 using DHW. No Stress means SST is at or below the site's MMM bleaching baseline (HotSpot ≤ 0);
Watch begins once SST exceeds it (0 < HotSpot < 1); Warning begins once HotSpot
reaches the 1 °C bleaching threshold. From there the NOAA CRW 5-level system
(expanded December 2023) is set by accumulated DHW: Alert Level 1 at 4, Level 2 at 8,
Level 3 at 12, Level 4 at 16, and Level 5 at 20 °C-weeks or more. Values above
8 °C-weeks are strongly associated with mass bleaching; values above 16 represent
conditions with limited historical precedent at most sites.
...
if n == 0:
    ...
    # NOAA CRW DHW bands (HotSpot >= 1 assumed once DHW is accumulating).
    # 0-4 is Watch or Warning depending on HotSpot; the split is not visible
    # on a DHW axis, so it is shaded as one band.
    alert_bands = [
        (0,  4,  "#FFF9C4", "Watch/Warning"),
        (4,  8,  "#FFCDD2", "Alert 1"),
        (8,  12, "#E1BEE7", "Alert 2"),
        (12, 16, "#F8BBD9", "Alert 3"),
        (16, 20, "#FFCCBC", "Alert 4"),
    ]
    ...
    for ax, name in zip(axes_flat, sites_to_plot):
        ...
        if ymax > 20:
            ax.axhspan(20, ymax, color="#CFD8DC", alpha=0.35, zorder=0)

        for thresh, label in [(4, "L1"), (8, "L2"), (12, "L3"),
                               (16, "L4"), (20, "L5")]:
            if thresh <= ymax:
...
Actions cron. To run locally, clone the
[el-nino-watch-2026](https://github.com/velezf/el-nino-watch-2026)
analysis repo, run `uv sync` (requires uv, Python 3.11) to restore the
environment, register the `el-nino-2026` kernel, and run `quarto render`
from the portfolio site root. The environment is fully specified via uv
