...
The baseline I keep running into is the CRW operational suite — HotSpot, DHW, Bleaching
Alert Area — built on the CoralTemp SST record and the Maximum Monthly Mean climatology
[@skirving2020; @liu2014]. These products appear to be the global standard and from what
I can tell reading the literature, they work. @heron2016 published what I take to be the
definitive 5km validation, demonstrating that the 5km products substantially outperform
the legacy 50km heritage products against in-water bleaching surveys.

Something I hadn't expected to find when I started reading is that CRW already operates
experimental products that go beyond temperature alone. The Light Stress Damage (LSD)
product suite combines temperature and light into a single stress index based on the
photoinhibition mechanism [@skirving2018]. CRW also publishes an experimental Four-Month
Coral Bleaching Outlook — updated weekly, forecasting the probability of bleaching-level
heat stress using SST projections from NOAA's CFSv2 ensemble [@liu2018]. The CRW
Thermal History product suite provides pre-aggregated annual thermal metrics per pixel
back to 1985. A reasonable question at this point is: if NOAA is already publishing
multi-factor experimental products, what is the point of building my own? The answer is
that CRW's products are designed for global operational monitoring — daily maps,
management alerts. What I am building is different: a regression-based, site-and-survey-
level risk score with the dependent variable rooted in published bleaching outcome
datasets and the independent variables combined at the analysis site level. That is a
different artifact than a global daily map, aimed at a different question.
...
metrics — 10,367 model permutations in total. The result I keep seeing cited everywhere
I look: high-frequency temperature variability is the single most predictive covariate,
and it has a *mitigating* effect. A 1°C increase in daily temperature range reduces the
odds of more severe bleaching by a factor of 33. That result directly shapes how I am
thinking about the thermal variability predictor in this project.
...
— to make their black-box models interpretable after the fact. I understand the appeal, but I am not starting there. ML methods are tools I want to
earn the right to use after there is a working regression baseline I can actually explain
from first principles. The spatial cross-validation approach in @cheung2025 is also
something I need to understand better before I try to implement it — it is on the reading
list before the implementation notebook gets written.
...
The mechanism is genuinely contested, and I want to be honest that I am still learning
it. The candidates are: selection pressure eliminating thermally sensitive genotypes over
successive bleaching events; acclimatization across the entire coral holobiont — the
coral animal plus its Symbiodiniaceae symbionts plus the bacteria, archaea, and other
microorganisms that live in and on the coral tissue as an integrated system; or
community-level shifts toward thermally tolerant symbiont clades like *Durusdinium*
[@palaciocastro2023]. As a reef keeper the holobiont framing clicked
...
**Mechanism:** From what I understand reading the literature, light and heat work
together to damage coral in a way that is more than additive. The biological pathway, as
I understand it from @warner1999 and @skirving2018, runs through the zooxanthellae —
the symbiotic algae living in coral tissue. Under heat stress, the algae's photosynthetic
machinery (specifically Photosystem II) starts breaking down faster than it can repair
itself, and the byproduct is reactive oxygen species that damage the coral tissue and
ultimately trigger expulsion of the symbionts — which is bleaching. The key point for
...
lower light would not.

I'll be honest that the photochemistry here is at the edge of what I can follow in
detail. What I am confident about is the practical implication: light exposure is a
modulating factor on thermal stress, there are published satellite products available,
and there is precedent in the multi-factor bleaching literature. NOAA CRW building an
experimental Light Stress Damage product on exactly this biology [@skirving2018] is what
convinced me PAR belongs in this model.
...
predictor — a straight line would miss the protective middle entirely. @sully2020 fit it
as a non-linear term, so I am following their lead on the quadratic specification.
...
surrounding seawater chemistry is for that mineral — from what I can tell reading
[@hoeghguldberg2007], values above 3 are generally considered good for coral
calcification, values between 2 and 3 are stressful, and values below 1 mean the water
is actively dissolving aragonite. This appears to be the standard chemical index for
ocean acidification stress on reefs.
...
few hundred meters across. I am joining a value computed at 100km scale to an outcome
observed at sub-1km spatial precision. My wife — a PhD economist with serious
econometric chops — flagged this as a textbook attenuation bias setup: the large-scale
measurement averages over spatial variation the model cannot see, which pulls the
coefficient toward zero relative to its true effect. The expectation going in is that the
Ω~ar~ coefficient will be underestimated, not that it will be zero. That is a meaningful
...
divided, and where my undergraduate background in evolutionary biology — I went to a
school with a strong evo-bio program, though I leaned molecular — gives me enough
footing to find the debate interesting rather than just confusing.
...
I have an unfair advantage on the econometrics side of this: my wife is a PhD economist
with serious econometric chops, and she has already flagged two things that warrant
expert review before any coefficients get reported publicly. The first is the dispersion
component specification — how variance scales with the mean in a beta regression is not
obvious and the diagnostic plots for this are not something I fully know how to read yet.
The second is the attenuation bias problem in the aragonite predictor, described above.
I plan to put that relationship to work. Whether to use the standard `betareg` fit or
the `glmmTMB` zero-inflated version is a decision I cannot make until the real data is
in hand and I can see how many exact zeros are in the Caribbean subset.
...
around the predicted event. @heron2016 used the same 25km / 4-week defaults for the CRW
5km validation — which makes this approach directly comparable to at least that published
benchmark, whatever else it turns out to be comparable to.
